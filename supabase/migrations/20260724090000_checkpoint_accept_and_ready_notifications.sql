-- Checkpoint accept/withdraw + post-ready notification history.
-- Faithful record of changes already applied to the live v2 database via the
-- Supabase MCP. Do NOT re-run against a hosted project. This file exists so a
-- fresh local or CI database reaches the same state as live.

-- 1. Accept columns + constraint + index on comments -------------------------
ALTER TABLE public.comments ADD COLUMN accepted_at timestamptz, ADD COLUMN accepted_by uuid REFERENCES auth.users(id) ON DELETE SET NULL, ADD COLUMN unaccepted_at timestamptz, ADD COLUMN unaccepted_by uuid REFERENCES auth.users(id) ON DELETE SET NULL;
ALTER TABLE public.comments ADD CONSTRAINT comments_accept_requires_resolved_checkpoint CHECK (accepted_at IS NULL OR (ledger_seq IS NOT NULL AND resolved_at IS NOT NULL));
CREATE INDEX comments_checkpoint_accept_idx ON public.comments (entity_id, ledger_seq) WHERE ledger_seq IS NOT NULL AND deleted_at IS NULL;

-- 2. Notification history table ----------------------------------------------
CREATE TABLE public.post_ready_notifications (id uuid PRIMARY KEY DEFAULT uuidv7(), workspace_id uuid NOT NULL REFERENCES public.workspaces(id) ON DELETE CASCADE, post_id uuid NOT NULL REFERENCES public.posts(id) ON DELETE CASCADE, sent_by uuid REFERENCES auth.users(id) ON DELETE SET NULL, checkpoint_count int NOT NULL, sent_at timestamptz NOT NULL DEFAULT now());
CREATE INDEX post_ready_notifications_post_sent_idx ON public.post_ready_notifications (post_id, sent_at DESC);
ALTER TABLE public.post_ready_notifications ENABLE ROW LEVEL SECURITY;
CREATE POLICY post_ready_notifications_select_member ON public.post_ready_notifications AS PERMISSIVE FOR SELECT TO authenticated USING (public.is_active_workspace_member(workspace_id));
...
-- 3. Widen inbox event_type check to admit 'checkpoint_reopened' --------------
...
AS $$
DECLARE v_ws uuid; v_ledger int; v_resolved timestamptz; v_deleted timestamptz;
BEGIN
  ...
  PERFORM public.audit_log_write('checkpoint_accept', 'success', p_trace_id, v_ws, 'comment',
          p_comment_id::text, jsonb_build_object('accepted', p_accepted));
END; $$;
...
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
DECLARE v_ws uuid; v_ledger int; v_parent uuid;
BEGIN
  ...
  IF v_parent IS NULL OR v_ledger IS NOT NULL THEN RAISE EXCEPTION 'invalid_payload'; END IF;
...
BEGIN
  ...
  IF NOT p_resolved AND v_rows > 0 AND v_ledger IS NOT NULL THEN
    ...
    SELECT wm.user_id, v_ws, 'checkpoint_reopened', v_entity_type, v_entity_id::text,
           'posts', v_entity_id::text, 'urgent', jsonb_build_object('comment_id', p_comment_id)
    FROM public.workspace_members wm
    WHERE wm.workspace_id = v_ws AND wm.active = true
      AND wm.user_id <> auth.uid() AND wm.role IN ('owner','admin','agency');
  END IF;
...
AS $function$
DECLARE v_ws uuid; v_ledger int; v_words int; v_parent uuid; v_parent_ledger int;
BEGIN
  SELECT workspace_id, ledger_seq, parent_comment_id INTO v_ws, v_ledger, v_parent FROM public.comments
   WHERE id = p_comment_id AND author_user_id = auth.uid() AND deleted_at IS NULL FOR UPDATE;
  IF v_ws IS NULL THEN RAISE EXCEPTION 'forbidden_role'; END IF;
  IF NOT public.is_active_workspace_member(v_ws) THEN RAISE EXCEPTION 'workspace_member_only'; END IF;
  IF p_body IS NULL OR length(p_body) < 1 OR length(p_body) > 10000 THEN RAISE EXCEPTION 'invalid_payload'; END IF;
  IF v_parent IS NOT NULL THEN
    SELECT ledger_seq INTO v_parent_ledger FROM public.comments WHERE id = v_parent;
  END IF;
  IF v_ledger IS NOT NULL OR v_parent_ledger IS NOT NULL THEN
...
DECLARE v_id uuid; v_scope text; v_is_draft boolean := false; v_brief_owner uuid; v_mentioned uuid[];
        v_parent_ledger int; v_words int;
BEGIN
  ...
  IF p_parent_comment_id IS NOT NULL THEN
    SELECT ledger_seq INTO v_parent_ledger FROM public.comments WHERE id = p_parent_comment_id;
  END IF;
  IF v_parent_ledger IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM public.workspace_members wm
                   WHERE wm.workspace_id = p_workspace_id AND wm.user_id = auth.uid()
                     AND wm.active = true AND wm.role = 'client') THEN
      RAISE EXCEPTION 'forbidden_role';
