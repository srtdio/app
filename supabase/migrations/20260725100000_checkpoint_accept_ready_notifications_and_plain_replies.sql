-- Checkpoint accept/withdraw, post-ready notification log, and plain checkpoint
-- replies. Applied to live v2 via the Supabase MCP; this file is the history-only
-- record so a fresh local or CI database (which replays supabase/migrations/ from
-- scratch) reaches the same state. Do NOT re-run against a hosted project.
--
-- Not DESTRUCTIVE: the only DROP is the named inbox_entries_event_type_check
-- CHECK, immediately re-added in widened form. No data is touched.

-- ============================================================================
-- 1. comments: accept / withdraw columns, guard, and lookup index
-- ============================================================================
...
-- ============================================================================
-- 2. post_ready_notifications: an append-only log of "post ready" fan-outs
-- ============================================================================
...
-- ============================================================================
-- 3. inbox_entries event_type enum: admit checkpoint_reopened
-- ============================================================================
...
-- ============================================================================
-- 4a. checkpoint_accept: a client confirms or withdraws a resolved checkpoint
-- ============================================================================
CREATE FUNCTION public.checkpoint_accept(p_comment_id uuid, p_accepted boolean, p_trace_id uuid)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
DECLARE v_ws uuid; v_ledger int; v_deleted timestamptz; v_resolved timestamptz; v_rows int := 0;
BEGIN
  SELECT workspace_id, ledger_seq, deleted_at, resolved_at
    INTO v_ws, v_ledger, v_deleted, v_resolved
...
-- ============================================================================
-- 4b. comment_delete: an author soft-deletes their own plain reply
-- ============================================================================
...
BEGIN
  ...
  PERFORM public.audit_log_write('comment_delete', 'success', p_trace_id, v_ws, 'comment',
          p_comment_id::text, jsonb_build_object('transitioned', true));
END; $$;
...
-- ============================================================================
-- 5a. comment_resolve: gate resolve to owner/admin/agency; reopen clears accept
--     and fans out checkpoint_reopened for a checkpoint thread
-- ============================================================================
...
-- ============================================================================
-- 5b. comment_edit: editing a checkpoint also clears accept state
-- ============================================================================
...
-- ============================================================================
-- 5c. comment_create: a reply to a checkpoint is an ordinary reply
-- ============================================================================
...
-- ============================================================================
-- 5d. comment_batch_create: drop all mention handling; ledger points only
-- ============================================================================
...
-- ============================================================================
-- 5e. post_ready_notify: log the fan-out before writing the inbox entries
-- ============================================================================
