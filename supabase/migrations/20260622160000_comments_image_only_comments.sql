...
ALTER TABLE public.comments ADD CONSTRAINT comments_body_check CHECK (char_length(body) <= 10000);
ALTER TABLE public.comments ADD CONSTRAINT comments_body_or_attachment_check
  CHECK (char_length(body) >= 1 OR coalesce(array_length(attachment_asset_ids,1),0) >= 1);
