...
| - | Chat UI (Chat shell): channel list + live message thread + plain-text composer, mounted under the Foundation ChatProvider on /chat only (connection torn down on navigate-away). Channel list from the Sorted registry (chat_channels via listChannels), display names resolved with batched group-name + DM-peer profile reads (no N+1), ordered created_at desc; the live thread reads history and incoming messages from the Agora SDK only via a separate chat-thread event handler, never the Postgres mirror; text send via the SDK. Connecting shows a loading state, unavailable a contained panel that never breaks the app. Responsive master-detail on existing primitives, light/dark parity, 44px touch targets. Last-message previews, unread counts, and the New chat action are deferred to later chat PRs. | 32, 17, 19 |
