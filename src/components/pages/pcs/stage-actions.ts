...
// client only ever sees Approve / Request changes and the agency side only ever
 ...
 *   rejected  -> "Request changes"  (client only)
...
export function visibleStageActions(stage: Stage, role: string | null): StageAction[] {
  ...
  for (const to of targets) {
    if (to === 'approved' && isClient(role)) {
      ...
      out.push({ to, label: 'Request changes', variant: 'default' });
